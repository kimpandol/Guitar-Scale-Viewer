...
import { analyzeMidiFile, MidiAnalysisResult, MidiScaleMatch } from './midi-analyzer';
//
// (html2canvas 전역 선언)
...
// ── MIDI 버튼 클릭 → 파일 선택 → 분석 흐름 ──
function initMidiAnalyzer(): void {
    const midiBtn = document.getElementById('midi-analyze-btn') as HTMLButtonElement | null;
    const midiInput = document.getElementById('midi-file-input') as HTMLInputElement | null;
    if (!midiBtn || !midiInput) return;
    ...
    midiInput.addEventListener('change', async () => {
        ...
        try {
            const result = await analyzeMidiFile(file);
            renderMidiAnalysisResult(result);
        } catch (err) {
            const analyzerResult = document.getElementById('scale-analysis-result');
            if (analyzerResult) {
